...
parallel distribution function depends only on parallel energy $W$
...
depends on $v$ through the $f_{\parallel0}$ distribution.  We can
...
\begin{equation}
  \label{eigq}
  (-\lambda_q+\lambda_\perp-\tilde V_w)a_q=\sum_j\bra{q}|\tilde{V}\ket{j}a_j
...
\end{equation}
The coefficient $(-\lambda_q+\lambda_\perp-V_w) $ is zero when
...
all prior bounces in the potential well, which requires a multiplying
resonant factor. Passing orbits use a single transit across the domain
