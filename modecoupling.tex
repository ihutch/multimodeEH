...
\begin{equation}
  \label{continw}
  0=(\lambda_s-\lambda_\perp)\bra{q}\ket{q}w_q+\bra{q}|\tilde{V}\ket{4}w_4
...
$\ket{q}$ in the final term ($\int\bra{q}|(\tilde{V}-\tilde{V}_{w})\ket{p}w_pdp $), it has order of
magnitude $\bra{q}|\tilde{V}-\tilde{V}_{w}\ket{q}\int w_pdp$. If,
therefore, $\int w_pdp$ is small compared with $w_4$, this integral
can be ignored compared with $\bra{q}|\tilde{V}\ket{4}w_4$, because
$\bra{q}|\tilde{V}-\tilde{V}_{w}\ket{q}$ and
...
\begin{equation}
  \label{eq:resonance}
  w_q = {\bra{q}|\tilde{V}\ket{4}w_4\over (\lambda_\perp-\lambda_q-\tilde{V}_{w})}
...
\iffalse
Let us be clear, however, that if the final term is not negligible,
then it is proportional to the magnitude of the the continuum
contribution. Therefore it belongs in the resonant denominator of the
expression for $w_q$, which becomes
$$
(\lambda_q-\lambda_\perp  +\tilde{V}_{w}
  +\bra{q}|(\tilde{V}-\tilde{V}_{w})\ket{q}\int w_pdp/w_q).
$$
To the extent that the resonance is narrow, the value of $w_q$ is
large. So near the resonance peak the extra term is not very
important, because it is of order the width.
\fi
