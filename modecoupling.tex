...
localized, tending to zero in the region beyond $z_l$.  We determine
$k_\parallel$ from eq.\ (\ref{eq:15}), the same for all harmonics. And
our numerical implementation ensures good cancelation by evaluating
$\Phi_m$ for passing orbits as $\ket{q}/i(k_\parallel v -\omega_m)$,
integrating the resulting $\tilde f_m$ $dv$ and summing over harmonics
$m$ to get $\tilde V_w\ket{q}$ externally. However, internally such an
treatment does not apply to trapped orbits, so instead we use
eq.\ (\ref{eq:24}) multiplying $\ket{q}$.
...
\begin{equation}
  ...
  {-i4\pi\bra{4}|\tilde V \ket{q_{0}}\bra{q_{0}}|\tilde{V}\ket{4}
    \over
    k_\parallel(1/\omega^2-1)\bra{4}\ket{4}C}.
\end{equation}
...
coupling to the continuum modes, which we do when
$\xi_t\gtrsim 1/2$.
