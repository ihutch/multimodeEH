...
values of $\psi$.
\begin{figure}
  \includegraphics[width=.48\hsize]{F2004B1p025}\hskip-2em(a)\quad
  \includegraphics[width=.48\hsize]{F2004B1p025d}\hskip-2em(b)\ 
...
negative imaginary in the intermediate regime. It is mostly real and
somewhat smaller in the high-field regime.
...
gives rather accurate eigenfrequencies.  The high-field regime
observation that the multimode is more stable seems more
counterintuitive.
...
\begin{figure}
  ...
  \caption{Contours of force balance quantities which should be zero
    ...
    both shiftmode and multimode contours. (b) Low field regime
    showing just multimode $||M||$ contours, but also the Newton steps
    to convergence.\label{hilow}}
\end{figure}
...
force balance indicated $\tilde F-F_E$. The eigenfrequency is where
the two zero contours (blue and green solid lines) intersect, at very
small $\omega_i$. In addition, the zero contours of $||M||$, the
multimode equation, are plotted. Actually the real $||M||=0$ contour
is invisible, but lies just below the (corresponding) imaginary
$\tilde F-F_E=0$, while the imaginary $||M||=0$ contour practically
coincides with the real $\tilde F-F_E=0$ contour there, where
$\omega_i$ is small. The two calculations agree on the eigenfrequency
very closely.

The right hand case \ref{hilow}(b) is, by contrast, in the low field
regime in which the instability is purely growing. Contours of $||M||$
are shown; the zero contours intersect only because the zero real part
contour, after crossing the lower left part of the plot, proceeds up
the vertical axis. The Newton iterations, which find this intersection
precisely, are indicated with iteration numbers at the corresponding
frequencies. The rather messy contours at the right of this plot arise
where $\omega_r$ becomes comparable to $\Omega$ and the external wave
(continuum modes) dispersion breaks down. Those effects, and other
rounding problems visible at low $\omega_i$, do not matter because the
eigenfrequency does not lie there.
...
intersections, the eigenfrequencies in the vicinity of
$\omega/\sqrt{\psi}=0.11+.01i$, for multimode and shiftmode. At the
right (b) are shown plots of the corresponding multimode potential
perturbation as a function of position $z$.  In the upper frame one
sees that in addition to mode 4 contribution, which far exceeds the
vertical axis length, there is a substantial (up to 0.08) real
contribution from mode 2. The resultant total (real) mode shape is
shown in the lower panel by the solid line, with the mode 4
...
been noted before\cite{Hutchinson2019}, the deeply trapped particles drive the
instability in this regime, while the marginally trapped tend to
stabilize it.
