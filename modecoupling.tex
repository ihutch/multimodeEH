...
Let us adopt a minimalist bra-ket notation in which the eigenmodes in
which we expand our solutions are
...
\begin{equation}
  ...
  \label{eq:4}
\end{equation}
in the sense that then $\bra{p}\ket{q}=\delta_{pq}=\delta(p-q)$. The
eigenmodes are plotted in Fig.\ \ref{modeplots}.
\begin{figure}\center
...
for the continuum modes, $\tilde{V}$ is not systematically small, and an
altered approach appropriate to the actual form of $\tilde{V}$ must be
adopted.
...
\end{equation}
(In our normalized units $\omega_{pe}=1$.)
Assuming that $\omega$ and $k_\perp$ are prescribed, there is
therefore only one $k_\parallel$ that satisfies this dispersion
relation; and the continuum eigenmode of $V_a$ that corresponds to it
has $p=4k_\parallel$ (using $x=z/4$). We shall write it with a compact complex
amplitude conveniently as $A{\rm e}^{ip|x|}$.
...
potential, from the external wave; obviously that $z$ is of order
4 (i.e.\ $x\sim 1$). The non-adiabatic perturbation to \emph{outward}
...
\ref{eq:16}.  Thus, even though the $\tilde f$ perturbations remain very
different for inward and outward going velocities, the densities are
not very different for $z>z_l$.
...
that factor $\tilde{V}_{ext}/16=-q_e[1+(k_\parallel v/\omega_m)^2]$. Then
...
$\ket{q}$ in the local integral term, it has order of magnitude
$\bra{q}|\tilde{V}-\tilde{V}_{ext}\ket{q}\int w_pdp$. If, therefore,
$\int w_pdp$ is small compared with $w_4$, this integral can be
ignored compared with $\bra{q}|\tilde{V}\ket{4}w_4$, because
...
of (\ref{eq:contadiab}), $\bra{q}|\tilde{V}\ket{4}w_4$, and (\ref{eq:18})
to zero and find
\begin{equation}
  \label{eq:resonance}
...
\end{equation}
by symmetry and the total eigenvalue equation, dropping symmetric discrete modes, becomes
\begin{equation}
  \label{eigtotal}
...
$ v_{t\perp}$, and $k_\perp\ll 1$ (long perpendicular wavelength
compared to hole length) the main effect is to broaden the
$q$-resonance by a relative width
$\delta \omega/\omega \sim3k_\perp v_{t\perp}\ll 1 $. We will assume
...
as to initialize the $\Phi_m$ integration into the hole. This is
provided by equation (\ref{eq:16}). The determination of
$\tilde{V}\ket{q}$ through the rest of the hole is accomplished with
by the internal integration over passing orbits, providing
...
all that happens at a particular velocity is that its $\tilde f$ phase
...
orbits of opposite $z$ and $v$ sign, and add the internal hole
integral, to give the total $\bra{q}|\tilde V \ket{4}$.
