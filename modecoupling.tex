...
\end{equation}
But we use the analytic form only to determine $k_\parallel$ from eq.\
(\ref{eq:15}), the same for all harmonics. Our numerical
implementation actually evaluates $\Phi_m$ as
$\ket{q}/i(k_\parallel v -\omega_m)$, integrates the resulting
$\tilde f_m$ $dv$ and sums over harmonics $m$ to get
$\tilde V_w\ket{q}$.  The crucial point is that
$(\tilde{V}-\tilde{V}_{w})\ket{q}$ is localized, tending to zero in
the region beyond $z_l$.
